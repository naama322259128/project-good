...
const PublicationApproval = (props) => {
    ...
    useEffect(() => {
        ...
        localStorage.setItem("publicationApproval", false);
    ...
    useEffect(() => {
        return () => {
            ...
            savePublicationApprovalInDB(props.auctionId, localStorage.getItem("publicationApproval")).then(succ => {
                if (succ.status != 400) props.setNewAuction(succ.data);
            })
    localStorage.removeItem("publicationApproval");
    ...
    <>
        ...
        <FormControlLabel
            control={<Checkbox checked={publicationApproval}
                ...
                disabled={!succses}
                onChange={(e) => {setPublicationApproval(e.target.checked);localStorage.setItem("publicationApproval", e.target.checked)}} />}
